...
class ChameleonAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_norm = ChameleonLayerNorm(
            self.head_dim, 1, self.num_kv_heads
        )
...
class ChameleonForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def sample(
        ...
        """
            Assume that the number of logits is 2 * batch size and for each two consecutive
            logits represent the logits for conditioned, unconditioned respectively.
            To match the generated tokens from two different logits, we force to the second one
            is the same as the first one.
        """
        
        next_tokens = self.sampler(logits, sampling_metadata)
        
        for i in range(0, len(next_tokens), 2):
            next_tokens[i+1].samples[0].output_token = next_tokens[i].samples[0].output_token
            next_tokens[i+1].samples[0].logprobs = next_tokens[i].samples[0].logprobs

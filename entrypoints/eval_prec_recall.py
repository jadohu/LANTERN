# This code is from https://github.com/youngjung/improved-precision-and-recall-metric-pytorch/blob/master/improved_precision_recall.py
#!/usr/bin/env python3
import os
...
def parse_args(args):
    parser = ArgumentParser(description='Evaluate precision and recall', formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument('--ref_dir', type=str, help='Path to the real images')
    parser.add_argument('--fake_dir', type=str, help='Path to the fake images')
...
def run_eval_prec_recall(args):
    ipr = IPR(args.batch_size, args.k, args.num_samples)
    with torch.no_grad():
        # real
        ipr.compute_manifold_ref(args.ref_dir)

        if len(args.fname_precalc) > 0:
            ipr.save_ref(args.fname_precalc)
            print('fake_dir (%s) is ignored for precalc' % args.fake_dir)
            exit()

        # fake
        precision, recall = ipr.precision_and_recall(args.fake_dir)

...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            down_proj = self.ffn_dropout(down_proj)
        else:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if cfg is not None:
            cond_null = torch.zeros_like(c_indices, device=c_indices.device) + self.model.cls_embedding.uncond_embedding.to(c_indices.device)

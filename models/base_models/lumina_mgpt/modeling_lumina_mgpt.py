...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    # Copied from `transformers.generation.utils.GenerationMixin._sample` to allow for custom new token for parallel CFG
    def _sample(
        self,
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            """
                In the parallel CFG mode, we need to ensure that the input_ids are same in both sequences (conditional and unconditional)
                Since the `transformers.generation.utils.GenerationMixin._sample()` can produce different input_ids even the logits are same,
                we need to manually set the input_ids to the same value.
            """
            if hasattr(self, 'cfg_mode') and self.cfg_mode == 'parallel':

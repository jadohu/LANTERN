...
class TqdmStoppingCriteria(StoppingCriteria):
    def __init__(self, total_tokens):
        super().__init__()
        self.pbar = tqdm(total=total_tokens)
...
class BatchedClassifierFreeGuidanceLogitsProcessor(LogitsProcessor):
    ...
    The unconditional scores are given as input to the processor.
    """

    def __init__(
        self,
        guidance_scale: float,
        model, # redundant; placeholder for matching the input arguments with the unbatched version
    ...
    def __call__(self, input_ids, scores):
        """
        Args:
            input_ids: torch.LongTensor of shape (batch_size * 2, sequence_length)
            scores: torch.FloatTensor of shape (batch_size * 2, sequence_length, vocab_size)
        
        Premise:
            The input_ids tensor is a concatenation of the prompt conditional and prompt unconditional input_ids.
            The scores tensor is a concatenation of the prompt conditional and prompt unconditional logits.
            The prompt conditional input_ids are the first half of the input_ids tensor.
            The prompt unconditional input_ids are the second half of the input_ids tensor.
            The prompt conditional logits are the first half of the scores tensor.
            The prompt unconditional logits are the second half of the scores tensor.
        """
        ...
        if num_image_start_tokens == num_image_end_tokens:
            ...
            if new_token_num >= 2:
                ...
                cond_logits, uncond_logits = scores.chunk(2, dim=0)
                scores_processed = self.guidance_scale * (cond_logits - uncond_logits) + uncond_logits
                scores_processed = scores_processed.repeat(2, 1)
...
class FlexARInferenceSolver:
    ...
    def __init__(self, model_path, precision, target_size=512, cfg_mode="sequential"):
        ...
        self.model.cfg_mode = cfg_mode
        if cfg_mode == "parallel":
            self.model.image_start_token_id = 8197
    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.cfg_mode == "parallel":
            image_start_sequence = torch.tensor([[8197, 8828, 8828]], dtype=torch.long, device=prompt.device)
            prompt = torch.cat([prompt, image_start_sequence], dim=-1)
            prompt = prompt.repeat(2, 1)

            prompt[1, :prompt_len] = 1 # 1 is the <pad> token
            attn_mask = torch.where(prompt == 1, False, True).to(prompt.device)
        else:
            attn_mask = None
        ...
        if logits_processor is None:
            logits_processor = self.create_logits_processor(cfg=cfg_scale, image_top_k=top_k)
        
        if tqdm:
            tqdm_logger = TqdmStoppingCriteria(total_tokens=max_gen_len)
            tqdm_loggers = StoppingCriteriaList([tqdm_logger])
        else:
            tqdm_loggers = None
    ...
    def create_logits_processor(self, cfg=3.0, image_top_k=2000, text_top_k=10):
        ...
        if self.cfg_mode == "parallel":
            CFGLogitsProcessor = BatchedClassifierFreeGuidanceLogitsProcessor
        else:
            CFGLogitsProcessor = LLMImageStartTriggeredUnbatchedClassifierFreeGuidanceLogitsProcessor
        
        cfg_processor = CFGLogitsProcessor(
            guidance_scale=cfg,
            model=self.model,
            image_start_token_id=self.item_processor.token2id(self.item_processor.image_start_token),
            image_end_token_id=self.item_processor.token2id(self.item_processor.image_end_token),
